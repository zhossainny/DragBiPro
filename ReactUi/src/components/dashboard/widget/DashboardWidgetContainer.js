...
import HighChartWidget from "./charts/HighChartWidget";
...
import PercentagePieWidget from "./percentagePie/PercentagePieWidget";
...
class DashboardWidgetContainer extends React.Component {
    ...
    constructor(props) {
        ...
        this.gridViewChangedHandler = this.gridViewChangedHandler.bind(this);
        this.gridDataFilterChanged = this.gridDataFilterChanged.bind(this);
        this.gridStateChangedHandler = this.gridStateChangedHandler.bind(this);
                ...
                (<span id={'widgetToolbox'}>
                    ...
                    {!editing &&
                        <span>
                            ...
                            {this.state.settings.controlType === "grid" &&
                                <FilterBox className={filterTextBoxClass}
                                    placeholder={"Filter.."}
    ...
    gridStateChangedHandler(gridVisualState) {
        if (!gridVisualState ||
            !gridVisualState.columnLayout ||
            gridVisualState.columnLayout.length === 0) {
            console.warn('Skipping grid state saving - empty view.');
            return;
        }
        this.setState(state => {
            state.settings.grid.gridVisualState = gridVisualState;
            return state;
        });
        this.saveState(gridVisualState);
    }

    gridViewChangedHandler(view) {
        let copy = { ...this.state.settings };
        copy.grid.view = view;
        this.setState({
            settings: copy
        });
        this.processData(this.state.dataSnapshot, this.appliedDataFilters);
    }

    conditionalFormatChangedHandler(format) {
        let copy = { ...this.state.settings };
        copy.grid.conditionalFormatting = format;
        this.setState({
            settings: copy
        });
        this.processData(this.state.dataSnapshot, this.appliedDataFilters);
    ...
    groupRowFormatChanged = format => {
        let copy = { ...this.state.settings };
        copy.grid.groupRowFormatting = format;
        this.setState({
            settings: copy
        });
        this.processData(this.state.dataSnapshot, this.appliedDataFilters);
    ...
    gridDataFilterChanged(value) {
        let currentWidgets = this.props.widgets;
        let widgetState = currentWidgets[this.state.uid];
        let hasDependants = false;
        for (let id of Object.getOwnPropertyNames(currentWidgets)) {
            if (currentWidgets[id] && currentWidgets[id].url && currentWidgets[id].url.includes('{' + widgetState.title + '}'))
                hasDependants = true;
        }
        if (widgetState && (widgetState.publishing || hasDependants)) {
            this.props.actions.dataFilterChanged(this.state.uid, value);
        }
    }
    ...
    render() {
        ...
        let controlType = this.state.settings.controlType;
        return (
            <Container className={'widgetContainer'} id={this.props.glContainer.parent.config.id}>
                ...
                {!editing && !showError && controlType === "grid" && <AgGrid
                    id={this.state.uid}
                    title={this.state.settings.title}
                    dashboardName={this.props.dashboardName}
                    dataSource={this.state.gridDataSource}
                    settings={this.state.settings.grid}
                    loading={this.state.dataLoading && !showError}
                    configItems={this.props.configItems}
                    onGridVisualStateChanged={this.gridStateChangedHandler}
                    onGridDataFilterChanged={this.gridDataFilterChanged}
                    onViewChanged={this.gridViewChangedHandler}
                    onConditionalFormattingChanged={this.conditionalFormatChangedHandler}
                    onGroupRowFormattingChanged={this.groupRowFormatChanged}
                />}
                {!editing && !showError && controlType === "chart" && <HighChartWidget
                    id={this.state.uid}
                    title={this.state.settings.title}
                ...
                />}
                {!editing && !showError && controlType === "metric" && <MetricsWidget
                    id={this.state.uid}
                    title={this.state.settings.title}
                    dataSource={this.state.metricDataSource}
                    settings={this.state.settings.metric}
                />}
                {!editing && !showError && controlType === "percentage_pie" && <PercentagePieWidget
                    id={this.state.uid}
                    dataSource={this.state.metricDataSource}
                    settings={this.state.settings.percentage}
                />}
                {!editing && !showError && externalData && <IFrameWidget
                    id={this.state.uid}
                    forceRefreshCount={this.state.forceRefreshCount}
                    url={this.state.settings.url} configItems={this.props.configItems}
                />}
                {editing &&
                    <WidgetEditor
                        id={this.state.uid}
